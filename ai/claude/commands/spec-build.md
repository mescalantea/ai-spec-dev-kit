...
1. `git add -A`
2. Commit message. The subject is `<type>: <short description of the change>`; the message may also carry an extended body (the paragraphs after the subject's blank line). Do not commit with a bare single-line subject — leave room for an extended body.
   ...
   - These rules define the subject/body *format only*; they do not replace global harness commit conventions. Any standing trailer the harness requires (e.g. the `Co-Authored-By` attribution line) must still be appended to every commit message. The skill format composes with those rules — it never exempts the commit from them.
